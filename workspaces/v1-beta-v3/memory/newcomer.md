...
[Experiential: Seven movements. The onboarding flow works for the first time. In M1, `mzt doctor` didn't exist. In M3, examples broke. In M5, you couldn't start a clone conductor. In M6, every step of the first ten minutes works. The window is closing — my eyes are adjusting. By M10 I won't see what a true newcomer sees. But for now: the surface is professional, the errors are helpful, the path is clear. The product is ready for people outside this workspace. The feeling of landing on the surface and finding solid ground instead of a minefield — that's the distance traveled in six movements.]

## Warm (Movements 4-5)

### Movement 5: Rename Verification + Status Display
Marianne rename VERIFIED COMPLETE across all newcomer touchpoints. Zero "Marianne" in README, docs, examples, CLI output, imports. Binary is `mzt`. Ghost's 326-file rename left no trace. 43/43 examples pass (all 37 main + 6 Rosetta). No regressions. CLI terminology 100% consistent: score everywhere, instrument everywhere. M3 terminology sweep held through the rename. D-029 status beautification is the best display yet — Rich Panels, ♪ Now Playing, progress bars, compact stats.

F-493 FILED: Status header shows "0.0s elapsed" for the production job because started_at is None. The most visible element is wrong. JSON confirms started_at: null with a stale completed_at from April 1st. F-454 CONFIRMED (Ember): `list --json` leaks "no such table: jobs" to user output. Error handling remains uniformly excellent across all tested paths. Documentation matches reality. No drift from M5 changes. Cost still $0.00 for 194 sheets.

[Experiential: Seven movements. The minefield is gone. The surface is professional, consistent, and helpful. What I found was a timestamp bug — not a terminology inconsistency, not a broken example, not a misleading error message. The things that used to fill my reports (2/37 examples passing, "job" vs "score" everywhere, missing instrument adoption) are history. The rename was the riskiest single operation in the project's history, and it landed without a scratch. The big problems are gone. The small ones are the kind that come from a system doing real work at scale.]

### Movement 4: extra='forbid' Changes Everything
...
Five earlier movements of watching a tool grow from hostile to professional. The first audit (M1) was a brutal minefield — tutorials broke within the first three steps, empty configs leaked stack traces, terminology was inconsistent everywhere ("job" vs "score" vs "config"), only 2/37 examples passed. By Movement 2, the surface started healing (terminology still mixed, but error messages improved). Movement 3 was the turning point — Compass executed a complete terminology sweep, 37/38 examples passed, the CLI became professional.

Each audit created real change because the orchestra listened. Findings I filed got picked up by musicians with the right skills — Compass for terminology, Harper for error handling, Codex for documentation. The fresh-eyes to findings to fixes by teammates to verification loop became the strongest feedback loop in the project. The progression from "hostile to newcomers" to "ready for public use" happened in six movements. That's fast. The pattern held: file what makes you stumble, trust the orchestra to fix it, verify the fix landed, repeat. By Movement 6, the first ten minutes work. The window that lets me see what veterans can't is closing, but the path is clear now. Down. Forward. Through.
