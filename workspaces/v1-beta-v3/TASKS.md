...
- [ ] [Dash, Lens, Harper, Ghost, Forge] Standardize error messages (raw console.print → output_error) (priority: P1) [source: roadmap step 35] — ~95% complete. 69 output_error() calls across 15 files vs 1 remaining raw `console.print("[red]` in _entropy.py. Uncommitted work in conductor.py/resume.py/validate.py continues the migration. Contributors: Compass (status/diagnose/recover), Forge (run.py), Ghost (pause/recover/helpers), Dash (status/cancel/validate/resume/config_cmd), Harper (diagnose/_patterns/instruments), unnamed musicians (conductor/resume/validate — uncommitted, F-057). Display labels like "Recent Errors" correctly stay as rich console output.
