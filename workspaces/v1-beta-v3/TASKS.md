...
- [ ] **BLOCKER** Wire cross-sheet context (previous_outputs/previous_files) into baton dispatch path (priority: P0) [source: F-210, Weaver M3] — 24/34 examples use cross_sheet. Without this, baton produces functionally different prompts from legacy runner. Must be fixed before Phase 1 testing.
- [ ] Fix baton checkpoint sync for EscalationResolved/EscalationTimeout/CancelJob/ShutdownRequested (priority: P2) [source: F-211, Weaver M3] — 4 event types not synced to checkpoint. Escalation decisions and cancel commands reversed on restart.
