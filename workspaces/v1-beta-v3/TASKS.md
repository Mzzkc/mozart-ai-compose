...
- [ ] Audit existing examples/ scores against updated score-authoring skill — upgrade to use new features (fan-out aliases, per-sheet overrides, instrument terminology) where they make better examples. (priority: P2) [source: F-079 / composer notes] — Spark M2: modernized dialectic.yaml (movements: key, movement/voice aliases, parallel enabled) and parallel-research-fanout.yaml (movements: key, full voice/movement terminology, comprehensive comments). Both validate clean. Remaining: other fan-out examples (parallel-research.yaml, thinking-lab.yaml, worldbuilder.yaml, etc.).
