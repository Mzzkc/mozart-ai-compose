...
class MCPProxyService:
    ...
    def compress_tool_manifest(
        self,
        tools: list[MCPTool],
        level: str = "moderate",
    ) -> list[dict[str, Any]]:
        """Compress tool manifest for context optimization.

        Reduces token usage by truncating descriptions and removing
        optional schema fields.

        Args:
            tools: List of MCPTool objects
            level: Compression level (minimal, moderate, aggressive)

        Returns:
            Compressed tool definitions as dicts
        """
        compressed = []
        for tool in tools:
            if level == "aggressive":
                # Name only with minimal schema
                compressed.append({
                    "name": tool.name,
                    "description": tool.description[:30] if tool.description else "",
                    "inputSchema": self._compress_schema(tool.input_schema, "aggressive"),
                })
            elif level == "moderate":
                # Truncate description, remove examples
                compressed.append({
                    "name": tool.name,
                    "description": tool.description[:50] if tool.description else "",
                    "inputSchema": self._compress_schema(tool.input_schema, "moderate"),
                })
            else:
                # Minimal: just remove empty optional fields
                compressed.append({
                    "name": tool.name,
    ...
    def _compress_schema(
        ...
        """Compress a JSON schema by removing optional fields.
        ...
        compressed = {"type": schema.get("type", "object")}
        ...
        if level == "aggressive":
            ...
            # Keep all properties but simplify
            compressed["properties"] = {
                k: {
                    "type": v.get("type", "string"),
                    "description": v.get("description", "")[:30] if level == "moderate" else v.get("description", ""),
                }
                for k, v in props.items()
            }
