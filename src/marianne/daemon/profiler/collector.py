...
# How often (seconds) the collection loop applies retention cleanup.
# F-488: MonitorStorage.cleanup() is otherwise never called and monitor.db
# grows unbounded (~13MB/day, reached 577MB before this fix was wired).
    ...
    async def _collection_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                # 7. Periodic retention cleanup (F-488). Runs at most once
                # per _CLEANUP_INTERVAL_SECONDS so the storage DELETE +
                # WAL churn doesn't slow down the 5s snapshot tick.
                # Isolated in its own try/except so a cleanup failure
                # doesn't kill the collection loop.
                ...
                if (
                    ...
                    try:
                        await self._storage.cleanup(self._config.retention)
                    except Exception:
                        _logger.warning(
                            "profiler.retention_cleanup_failed", exc_info=True
                        )
                    # Update timestamp even on failure so we back off
                    # for the full interval instead of retrying every tick.
