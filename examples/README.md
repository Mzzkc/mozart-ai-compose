...
These scores were generated by the [Rosetta Score](../scores/the-rosetta-score.yaml) — Marianne's pattern discovery engine. Each demonstrates a named orchestration pattern from the [Rosetta Corpus](../scores/rosetta-corpus.md) with a real-world use case.

| Example | Pattern | Domain | Key Innovation |
|---------|---------|--------|----------------|
| [dead-letter-quarantine.yaml](rosetta/dead-letter-quarantine.yaml) | Dead Letter Quarantine | Product Engineering | Batch utility generation with quarantine for failed items |
| [echelon-repair.yaml](rosetta/echelon-repair.yaml) | Echelon Repair | Security | Tiered severity routing — cheap instruments for grep-level findings, expensive for architectural analysis |
| [immune-cascade.yaml](rosetta/immune-cascade.yaml) | Immune Cascade | Security | Graduated multi-tier response — cheap sweeps narrow to expensive investigation |
| [prefabrication.yaml](rosetta/prefabrication.yaml) | Prefabrication | Product Engineering | Parallel backend + frontend tracks validated independently against a shared interface contract |
| [shipyard-sequence.yaml](rosetta/shipyard-sequence.yaml) | Shipyard Sequence | Engineering | Validation gate prevents expensive fan-out on broken foundation |
| [source-triangulation.yaml](rosetta/source-triangulation.yaml) | Source Triangulation | Research | Verify claims from structurally independent sources — code, docs, tests |
